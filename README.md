# :hammer_and_wrench: Dotfiles (Fish Shell Edition)

Personal Arch Linux dotfiles for a **Sway** desktop, themed with
**gruvbox-material**. This is a fork of
[CelticBoozer/dotfiles](https://github.com/CelticBoozer/dotfiles) with the
shell switched from Zsh to **Fish**, an enhanced installation script, and a
few extra app configs.

This repository was reassembled from the fork so that it is a **complete**,
installable set of dotfiles again: the fork only contained the files that were
actually changed, so everything else (Sway/Waybar/kitty/mako/yazi/rofi
configs, wallpapers, icons, the Neovim config, system config backups, etc.)
has been brought back in from upstream, with the fork's changes re-applied on
top.

## :stars: Key differences from upstream

- **Fish** instead of Zsh (`ohmyzsh` removed, `.zshrc`/`.zshenv`/`.zsh_custom`
  removed; see `.config/fish/config.fish`).
- Enhanced `.bin/initial-installation.sh` with retry logic, logging to
  `~/.dotfiles-installation.log`, config backups, and extra validation.
- Updated package lists: `ffmpeg` instead of the deprecated `ffmpeg4.4`,
  `fish` added, `zsh` removed.
- Extra app configs not present upstream: VS Code (`.config/Code/User`),
  Discord (`.config/discord`), Dolphin gruvbox-material theme
  (`.config/dolphin`), and a Vivaldi gruvbox-material stylesheet
  (`.config/vivaldi`).

## :brain: Core system info

- OS: [Arch Linux](https://archlinux.org/)
- WM: [Sway](https://github.com/swaywm/sway/)
- Shell: [fish](https://fishshell.com/)
- Terminal Emulator: [kitty](https://github.com/kovidgoyal/kitty/)
- Panel: [waybar](https://github.com/Alexays/Waybar/)
- Text Editor: [neovim](https://github.com/neovim/neovim/) (config in
  `.config/nvim`, pulled in from
  [CelticBoozer/nvim-config](https://github.com/CelticBoozer/nvim-config))
- App Launcher: [rofi](https://github.com/lbonn/rofi/)
- File Manager: [yazi](https://github.com/sxyazi/yazi/)
- Notification Manager: [swaync](https://github.com/ErikReider/SwayNotificationCenter/)
- Colorscheme: [Gruvbox-material](https://github.com/sainnhe/gruvbox-material/)

## :rocket: Installation

1. Fresh Arch Linux install, internet connection, `git` with SSH keys, sudo
   privileges.
2. Clone this repo and copy its contents to your home directory:

   ```bash
   git clone <this-repo-url>
   cd dotfiles
   cp -r . ~
   ```

3. Run the installer:

   ```bash
   bash ~/.bin/initial-installation.sh
   ```

4. Reboot: `sudo reboot`

See `~/.dotfiles-installation.log` after running for a detailed record of
what happened.

## :warning: Things to check before/after installing

- **`.system-config-backup/pacman/*.hook`** and
  **`.config/sway/config.d/daemons.conf`**: upstream hardcoded these to call
  `/bin/zsh -c '...'`; since zsh is no longer installed this repo switches
  them to `/bin/sh -c '...'` so the pacman hooks keep working. The pacman
  hooks also still hardcode the path `/home/celtic/...` from the original
  author — **update these to your own username** before installing, or the
  hooks will write to a path that doesn't exist on your system.
- **`.config/fish/config.fish`**: the fork's README described a Fish setup,
  but no fish config was ever committed. This file was generated by
  translating the equivalent settings from upstream's `.zshenv`/`.zshrc`
  (aliases, `$PATH` additions, env vars) into fish syntax. Review it and
  adjust to taste — in particular the JetBrains Toolbox vmoptions sourcing
  near the bottom is left commented out since it needs a fish-compatible
  version of that generated script.
- Two likely typos from the fork were corrected in the package lists:
  `vestktop-bin` → `vesktop-bin` (AUR) and `keeppassxc` → `keepassxc`
  (official repos) — neither package exists under the misspelled name.

## :heart: Acknowledgements

Base configuration and most of the heavy lifting: [CelticBoozer/dotfiles](https://github.com/CelticBoozer/dotfiles).
